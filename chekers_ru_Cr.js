...
function firstFilling() {
  ...
  for (let j = 0; j < 4; j++) {
    for (let i = 0; i < 4; i++) {
      const a = 0 + i + j;
      const b = 4 + i - j;
      specArr[0][4].cheker = 'wCr';
      specArr[1][3].cheker = 'b';
   ...
   //if (specArr[a][b - 1].num <= 12) specArr[a][b - 1].cheker = 'b';
     // if (specArr[a][b].num >= 21) specArr[a][b].cheker = 'w';
      if (specArr[a][b - 1].num >= 21) specArr[a][b - 1].cheker = 'b';
...
function moveCr(now, pre,color) {
  ...
  if (a === 'null' && b === 'wCr' && color == "wCr") {

    findSpecAr(now).cheker = 'wCr';
    findSpecAr(pre).cheker = 'null';
  
  }
 if (a === 'null' && b === 'bCr' && color == "bCr") {
